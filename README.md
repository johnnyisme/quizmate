...
- 文字/圖片任一即可送出
- **IME 輸入法支援**：完整支援注音、拼音、日文等輸入法，Enter 鍵選字不會誤觸發送訊息
- **完整 Markdown 支援**：標題、粗體、斜體、列表、程式碼區塊、表格、引用等
- **程式碼語法高亮**：支援多種程式語言，深色/淺色主題自動切換
- **KaTeX 數學公式渲染**：完整支援根號、分數、積分、矩陣等數學符號（已修復渲染管線順序問題）
...
- **多模型選擇**：Gemini 3 Flash、Gemini 3.1 Flash-Lite、Gemini 2.5 Flash-Lite、Gemini 2.5 Flash
...
7) **對話紀錄**：
   ...
   - **側邊欄滾動支援**：當對話列表過長時（30+ 個），支援垂直滾動瀏覽完整歷史
